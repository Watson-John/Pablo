...
class FilterDef {
  const FilterDef({required this.id, required this.label, required this.filter});
  final String id;
  final String label;
...
List<FilterDef> get kEditorFilters => [
      ...
      FilterDef(
          id: 'dramatic',
          label: 'Dramatic',
          filter: _saturate(1.25, 1.45)),
      const FilterDef(id: 'noir', label: 'Noir', filter: _noir),
      FilterDef(id: 'matte', label: 'Matte', filter: _saturate(0.6, 0.88, 1.06)),
...
ColorFilter _saturate(double s, [double c = 1.0, double b = 1.0]) {
  ...
  return ColorFilter.matrix(<double>[
    r1 * c * b, g1 * c * b, b1 * c * b, 0, 128 * (1 - c) * b,
    r2 * c * b, g2 * c * b, b2 * c * b, 0, 128 * (1 - c) * b,
    r3 * c * b, g3 * c * b, b3 * c * b, 0, 128 * (1 - c) * b,
    0, 0, 0, 1, 0,
  ]);
}
...
ColorFilter _hueShift(double degrees, double sat, double bright) {
  ...
  return ColorFilter.matrix(<double>[
    r1 * bright + warmBias * 20, g1 * bright, b1 * bright, 0, 0,
    r2 * bright, g2 * bright, b2 * bright, 0, 0,
    r3 * bright, g3 * bright, b3 * bright + coolBias * 20, 0, 0,
    0, 0, 0, 1, 0,
  ]);
}
