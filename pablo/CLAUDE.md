...
5. **No new pub deps** beyond what's already in `pubspec.yaml`, except the approved exceptions: `google_fonts: ^6.2.1` (DM Sans / Lora / JetBrains Mono at runtime), `file_selector: ^1.0.3` (native folder picker for Relocate/Export), `crypto` (model-download SHA-256), and — added in §10 Stage V2 — `share_plus` (OS share sheet / NSSharingServicePicker), `printing` (native print dialog), and `pdf` (pure-Dart print-layout builder).
