...
2. **Modular.** Each feature lives in its own folder under `lib/features/`; a feature's page file is LAYOUT ONLY — every reusable widget goes in the feature's `widgets/` subfolder as a public class. Cross-feature primitives live in `lib/components/` (notably `HoverSurface` — never hand-roll a `bool _hover` + MouseRegion machine again — and `DialogOptionTile`). No monolithic files (target ≤ 250 lines).
