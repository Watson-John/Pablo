...
Engine::Engine(fs::path catalog_path,
    ...
    try {
        ...
        try {
            ...
            std::atomic_store(&edits_,
                              std::shared_ptr<const EditMap>(std::move(map)));
...
edit::EditEntry Engine::edit_lookup(int64_t asset_id) const {
    auto snap = std::atomic_load(&edits_);
    if (!snap) return {};
...
void Engine::store_edit_entry(int64_t asset_id, const edit::EditEntry* entry) {
    std::lock_guard<std::mutex> lk(edits_write_mu_);
    auto cur = std::atomic_load(&edits_);
    auto next = std::make_shared<EditMap>(cur ? *cur : EditMap{});
    if (entry) (*next)[asset_id] = *entry;
    else       next->erase(asset_id);
    std::atomic_store(&edits_, std::shared_ptr<const EditMap>(std::move(next)));
}
